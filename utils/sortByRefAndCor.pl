...
sub usage {
    ...
    print "\nUsage:\n";
    print "sortByRefAndCor.pl [--k POS] [--c COR] [--tmp dir] INPUT REF_DICT\n\n";
    ...
    print "  --c COR :   contig cordinate is in the field COR (1-based)\n";
    print "              of input lines.\n\n";
...
for ( my $i = 0 ; $i < $n ; $i++ ) {
	...
	my $cmd = "sort -k $cor,$cor"."n $tmp/sortByRef.$$.$i.tmp > $tmp/sortByRef.$$.$i.sort.tmp";
	system($cmd);
    my $f; 
    open ( $f, "< $tmp/sortByRef.$$.$i.sort.tmp" );
    while ( <$f> ) { print ; }
    close $f;

    unlink "$tmp/sortByRef.$$.$i.tmp";  
     unlink "$tmp/sortByRef.$$.$i.sort.tmp";   
}
